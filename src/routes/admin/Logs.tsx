...
const AuditLogsPage = () => {
  ...
  const [selectedLog, setSelectedLog] = useState<AuditLog | SecurityLog | null>(null);
  const [uniqueActions, setUniqueActions] = useState<string[]>([]);
  const [uniqueUsers, setUniqueUsers] = useState<Array<{ id: string; email: string; }>>([]);
  ...
  useEffect(() => {
    filterLogs();
  }, [auditLogs, securityLogs, activeTab, searchTerm, actionFilter, dateFilter, userFilter]);
  ...
  const loadLogs = async () => {
    try {
      ...
      const allUsers = [
        ...new Map([
          ...(auditData || []).filter(log => log.profiles).map(log => [log.user_id, { id: log.user_id!, email: log.profiles!.email }]),
          ...(securityData || []).filter(log => log.profiles).map(log => [log.user_id, { id: log.user_id!, email: log.profiles!.email }])
        ].filter(([id]) => id)).values()
  ...
  const calculateStats = (auditData: AuditLog[], securityData: SecurityLog[]) => {
    const allLogs = [...auditData, ...securityData];
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const todayLogs = allLogs.filter(log => {
      const logDate = new Date('timestamp' in log ? log.timestamp : log.created_at);
    ...
    <AdminLayout title="Audit Logs">
      ...
      <Card className="mb-6">
        <CardHeader>
          ...
          <CardDescription>Most frequent activities on the platform</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {activeTab === 'audit' ? 'Audit Logs' : 'Security Logs'} ({filteredLogs.length})
          </CardTitle>
          <CardDescription>
            {activeTab === 'audit' 
            ...
            <TableBody>
              {filteredLogs.map((log) => (
                <TableRow key={log.id}>
                  ...
                  <TableCell>
                    <div className="text-sm">
                      {formatTimestamp('timestamp' in log ? log.timestamp : log.created_at)}
                    </div>
                  </TableCell>
                  <TableCell>
                    <Dialog>
                          ...
                          <div className="space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-sm text-muted-foreground">{getUserName(selectedLog)}</p>
                              </div>
                            </div>
                            <div className="grid grid-cols-2 gap-4">
                              <div>
                                <label className="text-sm font-medium">Resource Type</label>
                                <p className="text-sm text-muted-foreground">{selectedLog.resource_type || 'N/A'}</p>
                              </div>
                              <div>
                                <label className="text-sm font-medium">Resource ID</label>
                                <p className="text-sm text-muted-foreground">{selectedLog.resource_id || 'N/A'}</p>
                              </div>
                            </div>
                            <div>
                            ...
                            {'error_message' in selectedLog && selectedLog.error_message && (
                              <div>
                                <label className="text-sm font-medium">Error Message</label>
                                <p className="text-sm text-red-600">{selectedLog.error_message}</p>
                              </div>
                            )}
                            {'metadata' in selectedLog && selectedLog.metadata && Object.keys(selectedLog.metadata).length > 0 && (
                              <div>
                            ...
                            <div>
                              ...
                              <p className="text-sm text-muted-foreground">
                                {formatTimestamp('timestamp' in selectedLog ? selectedLog.timestamp : selectedLog.created_at)}
                              </p>
                            </div>
                          </div>
                        )}
