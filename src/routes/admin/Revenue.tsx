...
interface RevenueData {
  ...
  topHosts: Array<{ host_id: string; host_name: string; revenue: number; bookings: number; }>;
  cityBreakdown: Array<{ city: string; revenue: number; bookings: number; }>;
  propertyTypeBreakdown: Array<{ type: string; revenue: number; bookings: number; }>;
  ...
  const loadRevenueData = async () => {
    try {
      ...
      (bookings || []).forEach(booking => {
        ...
        if (hostId) {
          const hostName = `${booking.property?.profiles?.first_name || ''} ${booking.property?.profiles?.last_name || ''}`.trim() 
            || booking.property?.profiles?.email || 'Unknown Host';
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-2">Track platform revenue and financial performance</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">${revenueData.commissionEarned.toLocaleString()}</div>
            <p className="text-xs text-muted-foreground">
              10% of total revenue
            </p>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Performance across different property categories</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {revenueData.propertyTypeBreakdown.map((type) => (
              <div key={type.type} className="p-4 border rounded-lg">
                ...
                <div className="text-2xl font-bold">${type.revenue.toLocaleString()}</div>
