...
export const validateAdvancedFilters = (
  ...
  if (filters.propertyTypes) {
    ...
    const invalidTypes = filters.propertyTypes.filter(
    ...
    if (filters.propertyTypes.length > 5) {
      warnings.push(
        'Selecting many property types may not effectively narrow results',
      );
    }
  }
  ...
  if (filters.amenities) {
    const invalidAmenities = filters.amenities.filter(
      (amenity) => !isValidAmenityId(amenity),
    );
  ...
  if (filters.bathrooms !== null) {
    if (filters.bathrooms < 1) {
      ...
      warnings.push(
        'Large number of bathrooms may significantly limit results',
      );
    }
  }
...
export const validateCombinedFilters = (
  ...
  if (
    ...
    warnings.push(
      `Advanced filter booking type (${advancedFilters.bookingType}) overrides search booking type (${searchFilters.bookingType})`,
    );
  }
  ...
  const restrictiveFactors = [
    ...
    advancedFilters.priceRange[1] - advancedFilters.priceRange[0] < 1000
      ? 1
      : 0,
    advancedFilters.propertyTypes && advancedFilters.propertyTypes.length <= 2
      ? 1
      : 0,
  ...
  if (restrictiveFactors >= 4) {
    warnings.push(
      'Many restrictive filters are applied - consider relaxing some criteria if you get few results',
    );
  }

  // Check for logical inconsistencies
  if (
    searchFilters.guests > 8 &&
  ...
  if (
    effectiveBookingType === 'monthly' &&
    searchFilters.checkIn &&
    searchFilters.checkOut
  ) {
    const daysDiff = Math.ceil(
      (new Date(searchFilters.checkOut).getTime() -
        new Date(searchFilters.checkIn).getTime()) /
        (1000 * 60 * 60 * 24),
    );
    if (daysDiff < 28) {
      warnings.push(
        'Monthly booking type with short date range may not find suitable properties',
      );
    }
  }
...
export const sanitizeSearchFilters = (
  filters: SearchFilters,
): SearchFilters => {
  return {
    ...filters,
    ...
    duration: filters.duration
      ? Math.max(1, Math.min(12, filters.duration))
      : undefined,
  };
};

export const sanitizeAdvancedFilters = (
  filters: AdvancedFilters,
): AdvancedFilters => {
  ...
  if (sanitized.priceRange) {
    ...
    sanitized.priceRange = [Math.max(0, min), Math.max(min + 10, max)];
  }
  ...
  if (sanitized.propertyTypes) {
    ...
    sanitized.propertyTypes = sanitized.propertyTypes.filter((type) =>
  ...
  if (sanitized.amenities) {
    sanitized.amenities = sanitized.amenities.filter((amenity) =>
      isValidAmenityId(amenity),
    );
