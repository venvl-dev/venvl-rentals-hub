    ...
    const fetchPriceData = async () => {
      try {
        ...
        const { data, error } = await supabase
          .from('properties')
          .select('price_per_night, daily_price, monthly_price, title, booking_types, rental_type')
          .eq('is_active', true)
          ...
          try {
            ...
            if (bookingType === 'daily') {
              ...
              const supportsMonthly =
                (Array.isArray(property.booking_types) && property.booking_types.includes('monthly')) ||
              ...
              if (
                ...
                console.log(`✅ Added monthly price: ${property.monthly_price}`);
              } else {
                console.log(`❌ Skipped monthly price (unsupported or invalid):`, {
                  monthly_price: property.monthly_price,
                  supportsMonthly
                });
              }
        ...
        console.log('Calculated price range:', { min: actualMin, max: actualMax, priceCount: prices.length });
      ...
      try {
        realtimeSubscriptionRef.current = supabase
          ...
          .on(
            ...
            (payload) => {
              console.log('Property change detected, refetching price range:', payload);
