...
const FilterBadgeDisplay = ({
  advancedFilters,
  onRemoveFilter,
  dbPriceRange,
}: FilterBadgeDisplayProps) => {
  ...
  const badges = useMemo(() => {
    ...
    if (
      ...
      try {
        badgeArray.push(
          ...
          >
            {formatPrice(advancedFilters.priceRange[0])} -{' '}
            {formatPrice(advancedFilters.priceRange[1])}
            <X
    ...
    if (
      ...
      const validAmenities = advancedFilters.amenities.filter(
    ...
    if (
      ...
      badgeArray.push(
        <Badge
          ...
          className='flex items-center gap-1'
        >
          {advancedFilters.bedrooms} bedroom
          {advancedFilters.bedrooms > 1 ? 's' : ''}
          <X
            className='h-3 w-3 cursor-pointer hover:text-destructive transition-colors'
    ...
    if (
      ...
      badgeArray.push(
        <Badge
          ...
          className='flex items-center gap-1'
        >
          {advancedFilters.bathrooms} bathroom
          {advancedFilters.bathrooms > 1 ? 's' : ''}
          <X
            className='h-3 w-3 cursor-pointer hover:text-destructive transition-colors'
