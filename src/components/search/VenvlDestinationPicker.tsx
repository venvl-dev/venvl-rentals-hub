...
const VenvlDestinationPicker = ({
  value,
  onChange,
  onClose,
}: VenvlDestinationPickerProps) => {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        ...
        console.log(
          '📍 Available properties (no current bookings):',
  ...
  useEffect(() => {
    if (searchTerm) {
      ...
      if (
        searchTerm &&
        !filtered.some((loc) => loc.toLowerCase() === searchTerm.toLowerCase())
      ) {
      ...
      const popularSuggestions = ['🌍 All Locations', ...featuredLocations];
