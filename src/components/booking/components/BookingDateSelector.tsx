...
const BookingDateSelector = ({
  ...
  console.log('📊 BookingDateSelector rendered with:', {
    bookingMode,
    checkIn,
    checkOut,
    monthlyStartDate,
  ...
  useEffect(() => {
    ...
    const documentDoubleClickHandler = (event: MouseEvent) => {
      ...
      if (dayButton) {
        console.log(
          '🌍 Document-wide double-click on calendar button detected',
        );
  ...
  const updateMonthlyDuration = (change: number) => {
    const newDuration = Math.max(
      minMonths || 1,
      Math.min(12, monthlyDuration + change),
    );
  ...
  const handleDateClick = (date: Date) => {
    ...
    if (
      ...
      console.log(
        '🔄 Double-click detected via handleDateClick - clearing selection',
      );
    ...
    <div className='space-y-4'>
      ...
      <AnimatePresence mode='wait'>
        {bookingMode === 'daily' && (
          ...
          >
            <div
              ref={dailyCalendarRef}
              className='relative overflow-hidden rounded-xl border border-gray-200/50 bg-gradient-to-b from-white to-gray-50/30 shadow-sm hover:shadow-md transition-all duration-300'
            >
              ...
              <div className='relative p-3 flex justify-center'>
                <Calendar
                  ...
                  classNames={{
                    ...
                    head_row: 'flex mb-2',
                    head_cell:
                      'text-black rounded-md w-10 font-medium text-sm uppercase tracking-wide',
                    row: 'flex mt-1 gap-1',
                    ...
                    day_selected:
                      'bg-gradient-to-br from-gray-800 to-black text-white border-gray-700 shadow-md hover:from-black hover:to-gray-800 [&[aria-selected=false]]:bg-transparent [&[aria-selected=false]]:text-black [&[aria-selected=false]]:border-transparent',
                    day_today: 'bg-white text-black border-black font-bold',
                    day_outside: 'text-gray-400 opacity-40',
                    day_disabled:
                      'bg-gray-100 text-gray-400 opacity-60 cursor-not-allowed border-gray-200',
                    day_range_middle:
                      'aria-selected:bg-gray-800 aria-selected:text-white aria-selected:border-black',
                  }}
                />
              </div>
            </div>
              ...
              >
                ...
                >
                  <span className='font-semibold'>
                    {differenceInDays(checkOut, checkIn).toLocaleString()}{' '}
                    nights
                  </span>
                  ...
                  {minNights &&
                    differenceInDays(checkOut, checkIn) < minNights && (
                      <div className='text-red-600 mt-2 text-sm font-medium'>
                        ⚠️ Minimum stay required: {minNights?.toLocaleString()}{' '}
                        night{minNights && minNights > 1 ? 's' : ''}
                      </div>
                    )}
                </div>
              </motion.div>
              ...
              <div className='relative p-3 flex justify-center'>
                <Calendar
                  ...
                  onSelect={(date) => {
                    ...
                    if (isDoubleClick) {
                      console.log(
                        '🚫 Ignoring onSelect due to double-click (monthly)',
                      );
                      return;
                    }
            ...
            <div className='bg-gray-50 rounded-lg p-3 space-y-3'>
              ...
              <div className='text-sm text-gray-600 text-center'>
                <span className='font-medium'>
                  {monthlyDuration.toLocaleString()} month
                  {monthlyDuration > 1 ? 's' : ''}
                </span>
                {monthlyStartDate && (
                ...
                {minMonths && monthlyDuration < minMonths && (
                  <div className='text-red-600 mt-2 text-sm font-medium'>
                    ⚠️ Minimum stay required: {minMonths?.toLocaleString()}{' '}
                    month{minMonths && minMonths > 1 ? 's' : ''}
                  </div>
                )}
              </div>
