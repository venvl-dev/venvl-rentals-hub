  ...
  const loadCalendarData = useCallback(async () => {
    ...
    try {
      ...
      if (userType === 'host') {
        ...
        if (hostProperties && hostProperties.length > 0) {
          ...
          for (
            ...
            const hasBlocked = daysForDate.some(
            ...
            combinedDays.push({
              ...
              blockedReason: daysForDate.find((day) => day.blockedReason)
                ?.blockedReason,
            });
          }
        }
        ...
        const queryStartDate = format(
          new Date(monthStart.getFullYear(), monthStart.getMonth() - 1, 1),
          'yyyy-MM-dd',
        );
        const queryEndDate = format(
          new Date(monthEnd.getFullYear(), monthEnd.getMonth() + 2, 0),
          'yyyy-MM-dd',
        );
        ...
        if (guestBookings && guestBookings.length > 0) {
          const propertyIds = [
            ...new Set(guestBookings.map((b) => b.property_id)),
          ];
          ...
          if (properties) {
            propertyTitles = properties.reduce(
              (acc, prop) => {
        ...
        calendarEndDate.setDate(
          calendarEndDate.getDate() + (6 - monthEnd.getDay()),
        ); // End on Saturday of last week
        const today = new Date();

        for (
          let d = new Date(calendarStartDate);
          d <= calendarEndDate;
          d.setDate(d.getDate() + 1)
        ) {
          ...
          const dayBookings =
            guestBookings?.filter((booking) => {
              try {
                ...
                return (
                  (isEqual(dayDate, checkIn) || isAfter(dayDate, checkIn)) &&
          ...
          combinedDays.push({
            ...
            bookingData: dayBookings[0]
              ? {
                  ...
                  property_title:
                    propertyTitles[dayBookings[0].property_id] || 'Property',
                }
              : undefined,
            isToday: format(d, 'yyyy-MM-dd') === format(today, 'yyyy-MM-dd'),
            isCurrentMonth: d.getMonth() === currentDate.getMonth(),
      ...
      const filteredDays = combinedDays.map((day) => {
        if (day.bookingData) {
          const matchesType =
            filterType === 'all' || day.bookingData.booking_type === filterType;
          if (!matchesType) {
            return {
              ...day,
              ...
              {day.bookingData && (
                <div className='space-y-1'>
                  ...
                  <div className='flex items-center gap-2'>
                    ...
                    <span className='text-sm'>
                      EGP {Math.round(day.bookingData.total_price)}
                    </span>
                  </div>
                  <Badge
                    className={`text-xs ${
                  ...
                  >
                    {day.bookingData.status === 'cancelled'
                      ? '❌ Cancelled'
                      : day.bookingData.status.charAt(0).toUpperCase() +
                        day.bookingData.status.slice(1)}
                  </Badge>
                </div>
              )}
