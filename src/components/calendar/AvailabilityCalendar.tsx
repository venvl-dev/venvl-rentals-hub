...
const AvailabilityCalendar = ({
  ...
  const fetchBookingsAndAvailability = async () => {
    try {
      ...
      const blocked =
        availabilityData?.map((item) => new Date(item.blocked_date)) || [];
  ...
  const isDateDisabled = (date: Date) => {
    if (!isHost) {
      ...
      const status = getDateStatus(date);
      return (
        status === 'booked' || status === 'blocked' || status === 'pending'
      );
    }
    return false;
  };
            ...
            <div className='space-y-4'>
              ...
              <div className='space-y-3'>
                ...
                <div className='flex justify-between'>
                  ...
                  <span className='font-medium'>
                    {format(parseISO(selectedBooking.check_in), 'MMM dd, yyyy')}
                  </span>
                </div>
                <div className='flex justify-between'>
                  ...
                  <span className='font-medium'>
                    {format(
                      parseISO(selectedBooking.check_out),
                      'MMM dd, yyyy',
                    )}
                  </span>
                </div>
