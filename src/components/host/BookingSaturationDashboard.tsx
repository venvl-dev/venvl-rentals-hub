...
import { CalendarDays, TrendingUp, Home, DollarSign, Users, BarChart3, Clock, CheckCircle, UserCheck, Award } from 'lucide-react';
import { format, subDays, startOfMonth, endOfMonth, differenceInDays, isWithinInterval } from 'date-fns';
...
const BookingSaturationDashboard = () => {
  ...
  const fetchSaturationData = async () => {
    try {
      ...
      const propertySaturation: PropertySaturation[] = propertiesData.map(property => {
        const propertyBookings = bookingsData?.filter(booking => booking.property_id === property.id) || [];
        const confirmedBookings = propertyBookings.filter(b => ['confirmed', 'checked_in', 'completed'].includes(b.status));
      ...
      const totalRevenue = propertySaturation.reduce((sum, prop) => sum + prop.revenue, 0);
      const totalBookings = propertySaturation.reduce((sum, prop) => sum + prop.bookingsCount, 0);
      const totalAvailableDays = propertySaturation.reduce((sum, prop) => sum + prop.totalDays, 0);
      const totalBookedDays = propertySaturation.reduce((sum, prop) => sum + prop.bookedDays, 0);
      const averageOccupancy = totalAvailableDays > 0 ? (totalBookedDays / totalAvailableDays) * 100 : 0;
      ...
      const topPerformer = propertySaturation.reduce((top, current) =>
    ...
    return (
      <Card className="rounded-3xl shadow-lg">
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-gray-600">Add active properties to view saturation metrics</p>
        </CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Track your property performance and occupancy rates</p>
        </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className={`text-2xl font-bold ${getOccupancyColor(metrics.averageOccupancy)}`}>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">EGP {metrics.totalRevenue.toLocaleString()}</p>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-yellow-800">{metrics.pendingBookings}</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-green-800">{metrics.confirmedBookings}</p>
        ...
        <Card className="rounded-2xl shadow-sm border-purple-200 bg-purple-50">
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-purple-700">Checked In</p>
                <p className="text-2xl font-bold text-purple-800">{metrics.checkedInBookings}</p>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold text-blue-800">{metrics.completedBookings}</p>
      ...
      <Card className="rounded-2xl shadow-sm bg-gradient-to-r from-blue-50 to-purple-50 border-blue-200">
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-medium text-blue-800">🏆 Top Performing Property</p>
              <p className="text-xl font-bold text-blue-900">{metrics.topPerformingProperty}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-gray-600">
                • You have {metrics.totalBookedDays} booked days out of {metrics.totalAvailableDays} available days
              </p>
              <p className="text-gray-600">
                • Average booking value across all properties: EGP {metrics.totalBookings > 0 ? (metrics.totalRevenue / metrics.totalBookings).toFixed(0) : '0'}
              </p>
            </div>
            <div>
              <p className="text-gray-600">
                • {properties.filter(p => p.occupancyRate >= 80).length} properties with 80%+ occupancy
              </p>
              <p className="text-gray-600">
                • {properties.filter(p => p.occupancyRate < 40).length} properties need attention (under 40% occupancy)
              </p>
            </div>
          </div>
        </CardContent>
