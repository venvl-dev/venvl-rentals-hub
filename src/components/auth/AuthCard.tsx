...
const AuthCard = ({ mode, onToggleMode, role }: AuthCardProps) => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!profile) {
        console.warn(
          'Profile not found for user, this should not happen with the trigger in place',
        );
        setErrors({
          general: 'User profile not found. Please contact support.',
        });
        return;
      }
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user && !data.user.email_confirmed_at) {
        toast.success(
          'Registration successful! Please check your email for the confirmation link to complete your account setup.',
        );
            ...
            {mode === 'signup' && !role && (
              <div>
                ...
                <RadioGroup
                  value={formData.role}
                  onValueChange={(value) =>
                    handleInputChange('role', value as AuthRole)
                  }
